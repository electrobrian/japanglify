...
| Compiler, CUDA toolkit, driver, and target architecture | Compiler `nvcc.exe` found at `C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v6.5\bin\nvcc.exe`, reporting `V6.5.13`; toolkit `CUDA v6.5` confirmed; WMI display driver `NVIDIA 341.92 (9.18.13.4192)` and `nvcuda.dll` `8.17.13.4192` / `NVIDIA CUDA 6.5.51 driver`; target architecture `sm_11` candidate, physical compatibility `[PENDING — requires physical device probe]` |
...
- CUDA Toolkit `v6.5` is installed and `nvcc.exe` is on PATH at
  ...
  The compiler reports CUDA release `6.5`, version `V6.5.13`.
  The readiness check found `cudart64_65.dll`, `cudart.lib`, and `nvcuda.dll`,
  but no CUDA samples or `deviceQuery.exe` executable.
